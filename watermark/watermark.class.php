...
class Watermark {
    ...
    private function watermark_bottom_right(&$image, &$watermark){
        ...
        switch ($this->imageType) {
            case 'gif':
                $result = imagegif ($image,$this->output_file);
                break;
            case 'jpg':
                $result = imagejpeg($image,$this->output_file,100);
                break;
            case 'png':
                $result = imagepng($image,$this->output_file);
                break;
            default:
              throw new CException('Invalid image file type');
              break;
    ...
    private function watermark_center(&$image, &$watermark){
        ...
        // imagecopymerge($image, $watermark_mini, $dest_x, $dest_y, 0, 0, $w, $h, 0);
        switch ($this->imageType) {
            case 'gif':
                $result = imagegif ($image,$this->output_file);
                break;
